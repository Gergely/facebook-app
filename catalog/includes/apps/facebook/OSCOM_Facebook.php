...
*/
  ...
  class OSCOM_Facebook {
    var $_code = 'facebook';
    var $_title = 'Facebook App';
    var $_version;
    var $_api_version = '100';
    ...
    function getReqApiCountries() {
      ...
      if ( !isset($countries) ) {
        ...
        foreach ( file(DIR_FS_CATALOG . 'includes/apps/facebook/req_api_countries.txt') as $c ) {
          $c = trim($c);
    ...
    function log($module, $action, $result, $request, $response, $server = 'live') {
      ...
      if ( in_array(OSCOM_APP_FACEBOOK_LOG_TRANSACTIONS, array('1', '0')) ) {
        $do_log = true;

        if ( (OSCOM_APP_FACEBOOK_LOG_TRANSACTIONS == '0') && ($result === 1) ) {
          $do_log = false;
        }
      }
      ...
      $data = array('customers_id' => tep_session_is_registered('facebook_login_customer_id') ? $facebook_login_customer_id : 0,
                    'module' => $module,
                    'action' => $action,
                    'result' => $result,
      ...
      tep_db_perform('oscom_app_facebook_log', $data);
    }
    ...
    function getModules() {
      ...
      if ( !isset($result) ) {
        ...
        if ( $dir = @dir(DIR_FS_CATALOG . 'includes/apps/facebook/modules/') ) {
          while ( $file = $dir->read() ) {
            if ( !in_array($file, array('.', '..')) && is_dir(DIR_FS_CATALOG . 'includes/apps/facebook/modules/' . $file) && file_exists(DIR_FS_CATALOG . 'includes/apps/facebook/modules/' . $file . '/' . $file . '.php') ) {
    ...
    function isInstalled($module) {
      if ( file_exists(DIR_FS_CATALOG . 'includes/apps/facebook/modules/' . basename($module) . '/' . basename($module) . '.php') ) {
        return defined('OSCOM_APP_FACEBOOK_' . basename($module) . '_STATUS') && tep_not_null(constant('OSCOM_APP_FACEBOOK_' . basename($module) . '_STATUS'));
      }
    ...
    function getModuleInfo($module, $info) {
      $class = 'OSCOM_Facebook_' . $module;

      if ( !class_exists($class) ) {
        $this->loadLanguageFile('modules/' . $module . '/' . $module . '.php');

        include(DIR_FS_CATALOG . 'includes/apps/facebook/modules/' . $module . '/' . $module . '.php');
      }
    ...
    function hasCredentials($module, $type = null) {
      if ( $type == 'email') {
        ...
        $creds = array('OSCOM_APP_FACEBOOK_API_CLIENT_ID',
    ...
    function getCredentials($module, $type) {
      if ( constant('OSCOM_APP_FACEBOOK_' . $module . '_STATUS') == '1' ) {
        if ( $type == 'email') {
          return constant('OSCOM_APP_FACEBOOK_SELLER_EMAIL');
        } else {
          return constant('OSCOM_APP_FACEBOOK_API_' . strtoupper($type));
        }
      }
    }
    ...
    function getParameters($module) {
      ...
      if ( $module == 'G' ) {
        if ( $dir = @dir(DIR_FS_CATALOG . 'includes/apps/facebook/cfg_params/') ) {
          while ( $file = $dir->read() ) {
            if ( !is_dir(DIR_FS_CATALOG . 'includes/apps/facebook/cfg_params/' . $file) && (substr($file, strrpos($file, '.')) == '.php') ) {
              $result[] = 'OSCOM_APP_FACEBOOK_' . strtoupper(substr($file, 0, strrpos($file, '.')));
            }
          }
        }
      } else {
        if ( $dir = @dir(DIR_FS_CATALOG . 'includes/apps/facebook/modules/' . $module . '/cfg_params/') ) {
          while ( $file = $dir->read() ) {
            if ( !is_dir(DIR_FS_CATALOG . 'includes/apps/facebook/modules/' . $module . '/cfg_params/' . $file) && (substr($file, strrpos($file, '.')) == '.php') ) {
              $result[] = 'OSCOM_APP_FACEBOOK_' . $module . '_' . strtoupper(substr($file, 0, strrpos($file, '.')));
            }
          }
        }
      }
    ...
    function getInputParameters($module) {
      ...
      if ( $module == 'G' ) {
        ...
        $cut = 'OSCOM_APP_FACEBOOK_' . $module . '_';
      }
      ...
      foreach ( $this->getParameters($module) as $key ) {
        ...
        if ( $module == 'G' ) {
          $cfg_class = 'OSCOM_Facebook_Cfg_' . $p;

          if ( !class_exists($cfg_class) ) {
            $this->loadLanguageFile('cfg_params/' . $p . '.php');

            include(DIR_FS_CATALOG . 'includes/apps/facebook/cfg_params/' . $p . '.php');
          }
        } else {
          $cfg_class = 'OSCOM_Facebook_' . $module . '_Cfg_' . $p;

          if ( !class_exists($cfg_class) ) {
            $this->loadLanguageFile('modules/' . $module . '/cfg_params/' . $p . '.php');

            include(DIR_FS_CATALOG . 'includes/apps/facebook/modules/' . $module . '/cfg_params/' . $p . '.php');
          }
        }
    ...
    function drawButton($title = null, $link = null, $type = null, $params = null, $force_css = false) {
      ...
      if ( isset($link) ) {
        $button .= '<a href="' . $link . '" class="fb-button';

        if ( isset($type) ) {
          $button .= ' fb-button-' . $type;
        }
        ...
        $button .= '<button type="submit" class="fb-button';
...
// fallback for v2.3.1
    ...
    function saveParameter($key, $value, $title = null, $description = null, $set_func = null) {
      if ( !defined($key) ) {
        if ( !isset($title) ) {
          $title = 'Facebook App Parameter';
        }

        if ( !isset($description) ) {
          $description = 'A parameter for the Facebook Application.';
        }
    ...
    function getVersion() {
      if ( !isset($this->_version) ) {
        $version = trim(file_get_contents(DIR_FS_CATALOG . 'includes/apps/facebook/version.txt'));
    ...
    function hasAlert() {
      return tep_session_is_registered('OSCOM_Facebook_Alerts');
    }

    function addAlert($message, $type) {
      global $OSCOM_Facebook_Alerts;

      if ( in_array($type, array('error', 'warning', 'success')) ) {
        if ( !tep_session_is_registered('OSCOM_Facebook_Alerts') ) {
          $OSCOM_Facebook_Alerts = array();
          tep_session_register('OSCOM_Facebook_Alerts');
        }

        $OSCOM_Facebook_Alerts[$type][] = $message;
      }
    }
    ...
    function getAlerts() {
      ...
      if ( tep_session_is_registered('OSCOM_Facebook_Alerts') && !empty($OSCOM_Facebook_Alerts) ) {
        $result = array();

        foreach ( $OSCOM_Facebook_Alerts as $type => $messages ) {
          if ( in_array($type, array('error', 'warning', 'success')) ) {
            $m = '<ul class="fb-alerts-' . $type . '">';
        ...
        if ( !empty($result) ) {
          $output .= '<div class="fb-alerts">' . implode("\n", $result) . '</div>';
        }
      }

      tep_session_unregister('OSCOM_Facebook_Alerts');

      return $output;
    }

    function install($module) {
      $cut_length = strlen('OSCOM_APP_FACEBOOK_' . $module . '_');
      ...
      $m_class = 'OSCOM_Facebook_' . $module;
    ...
    function uninstall($module) {
      tep_db_query("delete from " . TABLE_CONFIGURATION . " where configuration_key like 'OSCOM_APP_FACEBOOK_" . tep_db_input($module) . "_%'");
    ...
    function logUpdate($message, $version) {
      if ( is_writable(DIR_FS_CATALOG . 'includes/apps/facebook/work') ) {
        file_put_contents(DIR_FS_CATALOG . 'includes/apps/facebook/work/update_log-' . $version . '.php', '[' . date('d-M-Y H:i:s') . '] ' . $message . "\n", FILE_APPEND);
      }
    }
    ...
    public function loadLanguageFile($filename, $lang = null) {
      ...
      $pathname = DIR_FS_CATALOG . 'includes/apps/facebook/languages/' . $lang . '/' . $filename;
